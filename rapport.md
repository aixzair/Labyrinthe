Tests :
- test plus poussé dans labyrinth généateur : pièces, pièges
- repository, test dans la liste des noms et des scores
- tests des fonctions leaderbaord
- menus
- monsters : déplacement des monstres
- test user_interface
- game : test move
- game : améliorer test isGameOver
- game : améliorer test endGame
- game.h !!!!
- labyrinth_generator : test addObjectsInLabyrinth
- labyrinth_repository : test autres fonctions non testés
- scores : tout les tests

Jeu :
- problème déplacement monstres

Fonctionnalités :
- supprimer des labyrinth

Limites :
- nombres de labyrinthe
